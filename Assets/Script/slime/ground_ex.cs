...
public class ground_ex : MonoBehaviour
{
	private const int ground_max_x = 41;
	private const int ground_max_z = 41;
	...
	private void Start ()
	{
		for (int z = 0; z < ground_max_z; ++z) {
			for (int x = 0; x < ground_max_x; ++x) {
				...
				ground_array[x, z].GetComponent<Renderer>().material.color = Color.black;
			}
		}
...
//		cube.transform.position = new Vector3(0, 0.5F, 0);
	...
	private void Update ()
	{
		if (Input.GetKeyUp("t")) {
			...
			int wave_x = 23;//rnd.Next(ground_max_x);
			int wave_z = 23;//rnd.Next(ground_max_z);
			int wave_power = 150;
